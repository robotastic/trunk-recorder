...
| openmhzSystemId        |          | `shortName`                | string                                                                       | *if uploadServer is set* The OpenMhz plugin normally uploads to the `shortName` system.  Setting this value can allow uploads to any specific OpenMHz endpoint using its API key.  This is useful in a multi-site setup where several trunk-recoder systems may be aggregating calls to the same OpenMHz feed. | 
