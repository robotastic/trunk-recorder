...
| openmhzSystemId        |          | `shortName`                | string                                                                       | *if uploadServer is set* OpenMhz uploads to the `shortName` system by default.  Setting a value will override this behavior, and allow systems to upload to another OpenMHz endpoint using its API key.  This is useful in a multi-site setup where several systems may be sending calls to the same OpenMHz feed. | 
