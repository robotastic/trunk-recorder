#ifndef PFB_CHANNELIZER_CONVERSION_H
#define PFB_CHANNELIZER_CONVERSION_H
...
{
public:
    channelizer_hier_ccf(int n_chans, int n_filterbanks = 1, std::vector<float> taps = std::vector<float>(), std::vector<int> outchans = std::vector<int>(),
                         float atten = 100, float bw = 1.0, float tb = 0.2, float ripple = 0.1)
                          ...
                          gr::io_signature::make(outchans.size(), outchans.size(), sizeof(gr_complex)))
    {
        ...
        blocks::vector_map::sptr combiner = blocks::vector_map::make(sizeof(gr_complex), std::vector<int>(cpps.begin(), cpps.end()), combiner_mapping);
    ...
    static std::vector<float> create_taps(int n_chans, float atten = 100, float bw = 1.0, float tb = 0.2, float ripple = 0.1)
    {
        return optfir::low_pass(1, n_chans, bw, bw + tb, ripple, atten);
    }
};

#endif // PFB_CHANNELIZER_CONVERSION_H
