  ...
  try
  {
                  ...
                  pt.get_child("systems"))
    {
      ...
      system->set_talkgroups_file(node.second.get<std::string>("talkgroupsFile",
                                                               ""));
      BOOST_LOG_TRIVIAL(info) << "Talkgroups File: " <<
      system->get_talkgroups_file();
      systems.push_back(system);
    }
    config.capture_dir = pt.get<std::string>("captureDir",
    ...
    config_dir = pt.get<std::string>("configDir",
                                     boost::filesystem::current_path().string());
    ...
    boost::optional<std::string> mod_exists = pt.get_optional<std::string>(
      "modulation");
    ...
    if (mod_exists) {
      ...
      if (boost::iequals(system_modulation, "QPSK"))
      {
        ...
        "\tSystem Modulation Not Specified, assuming QPSK";
      }
    } else {
      qpsk_mod = true;
    }
                  ...
                  pt.get_child("sources"))
    {
      ...
      BOOST_LOG_TRIVIAL(info) << "Center: " <<
      node.second.get<double>("center", 0);
      BOOST_LOG_TRIVIAL(info) << "Rate: " << node.second.get<double>("rate", 0);
      BOOST_LOG_TRIVIAL(info) << "Error: " << node.second.get<double>("error", 0);
      BOOST_LOG_TRIVIAL(info) << "PPM Error: " <<
      node.second.get<double>("ppm", 0);
      ...
      BOOST_LOG_TRIVIAL(info) << "Digital Recorders: " << node.second.get<int>(
        "digitalRecorders",
        0);
      BOOST_LOG_TRIVIAL(info) << "Debug Recorders: " << node.second.get<int>(
        "debugRecorders",
        0);
      BOOST_LOG_TRIVIAL(info) << "Analog Recorders: " << node.second.get<int>(
        "analogRecorders",
        0);
      BOOST_LOG_TRIVIAL(info) << "driver: " << node.second.get<std::string>(
        "driver",
        "");
      ...
      if ((ppm != 0) && (error != 0)) {
        BOOST_LOG_TRIVIAL(info) <<
        "Both PPM and Error should not be set at the same time. Setting Error to 0.";
        error = 0;
      }
...
int start_recorder(Call *call, TrunkMessage message) {
  ...
  // BOOST_LOG_TRIVIAL(error) << "\tCall created for: " << call->get_talkgroup()
  ...
  if (call->get_encrypted() == false) {
    for (vector<Source *>::iterator it = sources.begin(); it != sources.end();
      ...
      if ((source->get_min_hz() <= call->get_freq()) &&
        ...
        if (call->get_tdma()) {
          BOOST_LOG_TRIVIAL(error) << "\tTrying to record TDMA: " <<
          call->get_freq() << " For TG: " << call->get_talkgroup();
          return 0;
        }
        ...
        if (talkgroup)
        {
          ...
          BOOST_LOG_TRIVIAL(error) << "\tTalkgroup not found: " <<
          call->get_freq() << " For TG: " << call->get_talkgroup();
    ...
    if (!source_found) {
      BOOST_LOG_TRIVIAL(error) <<
      "\tRecording not started because there was no source covering: " <<
      call->get_freq() << " For TG: " << call->get_talkgroup();
      return 0;
    }
  } else {
...
bool retune_recorder(TrunkMessage message, Call *call) {
  ...
  BOOST_LOG_TRIVIAL(info) << "\tRetune - Elapsed: " << call->elapsed() <<
  "s \tSince update: " << call->since_last_update() << "s \tTalkgroup: " <<
  message.talkgroup << "\tOld Freq: " << call->get_freq() << "\tNew Freq: " <<
  message.freq; // << "\tWav Pos: " << recorder->get_current_length() << "\tMsg
                // Q: " << msg_queue->count();
...
void assign_recorder(TrunkMessage message) {
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end();) {
    ...
    if (call_found && (call->get_talkgroup() == message.talkgroup)) {
      BOOST_LOG_TRIVIAL(info) << "\tALERT! Assign - Total calls: " <<
...
void current_system_id(int sysid) {
  ...
  if (active_sysid != sysid) {
    BOOST_LOG_TRIVIAL(info) << "Decoding System ID " << std::hex <<
    std::uppercase << sysid << std::nouppercase << std::dec;
    active_sysid = sysid;
  }
}
    ...
    if ((call->get_talkgroup() == message.talkgroup) &&
      ...
      if (call->get_freq() != message.freq) {
        ...
        if (call->get_state() == recording) {
          ...
          "\tOld Freq: " << call->get_freq() << "\tNew Freq: " << message.freq;
...
void unit_check() {
  ...
  path_stream << boost::filesystem::current_path().string() <<  "/" << 1900 +
  ltm->tm_year << "/" << 1 + ltm->tm_mon << "/" << ltm->tm_mday;
  ...
  sprintf(unit_filename, "%s/%ld-unit_check.json",
          path_stream.str().c_str(), starttime);
...
System* find_system(int sys_id) {
  ...
  for (std::vector<System *>::iterator it = systems.begin(); it != systems.end();
       ++it) {
  ...
  while (1) {
    ...
    if (sys) {
      if (sys->get_system_type() == "smartnet") {
        trunk_messages =
          smartnet_parser->parse_message(msg->to_string().erase(0, 10));
    ...
    if (timeDiff >= 3.0) {
      ...
      if (msgs_decoded_per_second < 10) {
        BOOST_LOG_TRIVIAL(error) << "\tControl Channel Message Decode Rate: " <<
        msgs_decoded_per_second << "/sec";
      }
    }
...
bool monitor_system() {
  ...
  for (vector<System *>::iterator it = systems.begin(); it != systems.end();
       it++) {
    System *system               = *it;
    bool    source_found         = false;
...
/*
  ...
  if (monitor_system()) {
    tb->start(1000);
    BOOST_LOG_TRIVIAL(info) << "Top Block Max number output items: " <<
    tb->max_noutput_items();
    ...
    BOOST_LOG_TRIVIAL(info) << "Unable to setup Control Channel Monitor" <<
    std::endl;
  }
