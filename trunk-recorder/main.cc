...
void load_config(string config_file) {
  ...
  try {
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type &node,
      ...
      BOOST_LOG_TRIVIAL(info)
          << "Squelch: " << node.second.get<double>("squelch", 0);
      BOOST_LOG_TRIVIAL(info)
          << "Idle Silence: " << node.second.get<bool>("idleSilence", 0);
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Control channel warning rate: " << config.control_message_warn_rate;
...
void process_message_queues() {
  for (std::vector<System *>::iterator it = systems.begin();
    ...
    for (std::vector<p25conventional_recorder_sptr>::iterator pit =
             sys->conventionalP25_recorders.begin();
         pit != sys->conventionalP25_recorders.end(); ++pit) {
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end();) {
    ...
    if (call->is_conventional() && call->get_recorder()) {
      ...
      if (call->get_current_length() > 0) {
        ...
        // if no additional recording has happened in the past X periods, stop
        // and open new file
...
void unit_check() {
  ...
  sprintf(unit_filename, "%s/%ld-unit_check.json", path_stream.str().c_str(),
          starttime);
        ...
        for (vector<Source *>::iterator src_it = sources.begin();
          ...
          if ((source->get_min_hz() <= channel) &&
            ...
            if (system->get_system_type() == "conventional") {
              ...
              rec = source->create_conventionalP25_recorder(
                  tb, system->get_delaycreateoutput());
              rec->start(call);
              call->set_recorder((Recorder *)rec.get());
              call->set_state(recording);
        ...
        if ((source->get_min_hz() <= control_channel_freq) &&
          ...
          if (system->get_system_type() == "p25") {
            ...
            system->p25_trunking = make_p25_trunking(
                control_channel_freq, source->get_center(), source->get_rate(),
                msg_queue, source->get_qpsk_mod(), system->get_sys_num());
            tb->connect(source->get_src_block(), 0, system->p25_trunking, 0);
          }
...
template <class F> void add_logs(const F &fmt) {
  boost::shared_ptr<
...
int main(int argc, char **argv) {
  ...
  if (config.log_file) {
    logging::add_file_log(keywords::file_name = "logs/%m-%d-%Y_%H%M_%2N.log",
