...
std::vector<TrunkMessage> P25Parser::decode_mbt_data(unsigned long opcode, boost::dynamic_bitset<> &header, boost::dynamic_bitset<> &mbt_data, unsigned long sa, unsigned long nac, int sys_num) {
  ...
  BOOST_LOG_TRIVIAL(info) << "decode_mbt_data: $" << opcode;
  if (opcode == 0x0) { // grp voice channel grant
    ...
    BOOST_LOG_TRIVIAL(info) << "mbt3b net stat: wacn " << std::dec << wacn << " syid " << syid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ") ";
    ...
    unsigned long ch1 = bitset_shift_mask(mbt_data, 80, 0xffff);
    unsigned long ch2 = bitset_shift_mask(mbt_data, 64, 0xffff);
    BOOST_LOG_TRIVIAL(info) << "mbt3c adjacent status "
    ...
    BOOST_LOG_TRIVIAL(info) << "mbt04\tUnit to Unit Chan Grant\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << format_freq(f) << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
  } else {
    BOOST_LOG_TRIVIAL(info) << "mbt other: " << opcode;
    return messages;
  }
...
std::vector<TrunkMessage> P25Parser::decode_tsbk(boost::dynamic_bitset<> &tsbk, unsigned long nac, int sys_num) {
  ...
  if (opcode == 0x00) { // group voice chan grant
    ...
    if (mfrid == 0x90) { // MOT_GRG_ADD_CMD
      ...
      BOOST_LOG_TRIVIAL(info) << "tsbk00\tMoto Patch Add \tsg: " << sg << "\tga1: " << ga1 << "\tga2: " << ga2 << "\tga3: " << ga3;
    ...
    if (mfrid == 0x90) {
      ...
      os << "tsbk02\tMoto Grant Update\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << format_freq(f) << "\tsg " << std::setw(7) << sg << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tsa " << sa;
      message.meta = os.str();
      BOOST_LOG_TRIVIAL(info) << os.str();
    } else {
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk04\tUnit to Unit Chan Grant\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << format_freq(f) << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk05\tUnit To Unit Answer Request\tsa " << sa << "\tSource ID: " << si;
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk06\tUnit to Unit Chan Update\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << format_freq(f) << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
  } else if (opcode == 0x08) {
    BOOST_LOG_TRIVIAL(info) << "tsbk08: Telephone Interconnect Voice Channel Grant";
  } else if (opcode == 0x09) {
    BOOST_LOG_TRIVIAL(info) << "tsbk09: Telephone Interconnect Voice Channel Grant Update";
  } else if (opcode == 0x0a) {
    BOOST_LOG_TRIVIAL(info) << "tsbk0a: Telephone Interconnect Answer Request";
  } else if (opcode == 0x14) {
    bool emergency = (bool)bitset_shift_mask(tsbk, 72, 0x80);
    bool encrypted = (bool)bitset_shift_mask(tsbk, 72, 0x40);
    bool duplex = (bool)bitset_shift_mask(tsbk, 72, 0x20);
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk14\tSNDCP Data Channel Grant\tsa " << sa << "\tChannels: " << chT << "/" << chR << " Freqs: " << format_freq(fT) << "/" << format_freq(fR) << " NSAPI: " << nsapi;
  } else if (opcode == 0x15) {
    BOOST_LOG_TRIVIAL(info) << "tsbk15: SNDCP Data Page Request";
  } else if (opcode == 0x16) {
    BOOST_LOG_TRIVIAL(info) << "tsbk16: SNDCP Data Channel Announcement -Explicit";
  } else if (opcode == 0x18) {
    BOOST_LOG_TRIVIAL(info) << "tsbk18: Status Update";
  } else if (opcode == 0x1a) {
    BOOST_LOG_TRIVIAL(info) << "tsbk1a: Status Query";
  } else if (opcode == 0x1c) {
    BOOST_LOG_TRIVIAL(info) << "tsbk1c: Messag Update";
  } else if (opcode == 0x1d) {
    BOOST_LOG_TRIVIAL(info) << "tsbk1d: Radio Unit Monitor Command";
  } else if (opcode == 0x1f) {
    BOOST_LOG_TRIVIAL(info) << "tsbk1f: Call Alert";
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk20\tAcknowledge Response\tga " << std::dec << ga << "\tsa " << sa << "\tReserved: " << op;
  } else if (opcode == 0x21) {
    BOOST_LOG_TRIVIAL(info) << "tsbk21: Extended Function Command";
  } else if (opcode == 0x24) {
    BOOST_LOG_TRIVIAL(info) << "tsbk24: Extended Function Command";
  } else if (opcode == 0x27) {
    BOOST_LOG_TRIVIAL(info) << "tsbk27: Deny Response";
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk2b\tLocation Registration Response\tga " << std::dec << ga << "\tsa " << sa << "\tValue: " << rv;
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk2c\tUnit Registration COMMAND\tsa " << std::setw(7) << sa << " Source ID: " << si;
  } else if (opcode == 0x2d) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk2d AUTHENTICATION COMMAND";
  } else if (opcode == 0x2e) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk2e DE-REGISTRATION ACKNOWLEDGE";
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk2f\tUnit Deregistration ACK\tSource ID: " << std::setw(7) << si;
  } else if (opcode == 0x30) {
    unsigned long mfrid = bitset_shift_mask(tsbk, 80, 0xff);
    if (mfrid == 0xA4) { // GRG_EXENC_CMD (M/A-COM patch)
      ...
      if (grg_a == 1) {   // Activate
        if (grg_g == 1) { // Group request
          ...
          BOOST_LOG_TRIVIAL(info) << "tsbk30 M/A-COM GROUP REQUEST PATCH sg TGID is " << sg << " patched with TGID " << ga;
        } else {
          message.message_type = PATCH_ADD;
          PatchData harris_patch_data;
          harris_patch_data.sg = sg;
          ...
          harris_patch_data.ga3 = ga;
          message.patch_data = harris_patch_data;
          BOOST_LOG_TRIVIAL(info) << "tsbk30 M/A-COM UNIT REQUEST PATCH sg TGID is " << sg << " patched with TGID " << ga;
        }
      } else {            // Deactivate
        if (grg_g == 1) { // Group request
          ...
          harris_patch_data.ga3 = ga;
          message.patch_data = harris_patch_data;
          BOOST_LOG_TRIVIAL(info) << "tsbk30 M/A-COM GROUP REQUEST PATCH DELETE for sg " << sg << " with TGID " << ga;
        } else {
          message.message_type = PATCH_DELETE;
          PatchData harris_patch_data;
          harris_patch_data.sg = sg;
          ...
          harris_patch_data.ga3 = ga;
          message.patch_data = harris_patch_data;
          BOOST_LOG_TRIVIAL(info) << "tsbk30 M/A-COM UNIT REQUEST PATCH DELETE for sg " << sg << " with TGID " << ga;
        }
      }
    } else {
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk31 AUTHENTICATION DEMAND";
  } else if (opcode == 0x32) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk32 AUTHENTICATION RESPONSE";
    ...
    if (mfrid == 0) {
      ...
      BOOST_LOG_TRIVIAL(info) << "tsbk33 iden up tdma id " << std::dec << iden << " f " << temp_chan.frequency << " offset " << temp_chan.offset << " spacing " << temp_chan.step << " slots/carrier " << temp_chan.slots_per_carrier;
    }
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk34 iden vhf/uhf id " << std::dec << iden << " toff " << toff * spac * 0.125 * 1e-3 << " spac " << spac * 0.125 << " freq " << freq * 0.000005 << " [ " << txt[toff_sign] << "]";
  } else if (opcode == 0x35) { // Time and Date Announcement
    BOOST_LOG_TRIVIAL(info) << "tsbk35 Time and Date Announcement";
  } else if (opcode == 0x36) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk36 ROAMING ADDRESS COMMAND";
  } else if (opcode == 0x37) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk37 ROAMING ADDRESS UPDATE";
  } else if (opcode == 0x38) { //
    BOOST_LOG_TRIVIAL(info) << "tsbk38 SYSTEM SERVICE BROADCAST";
    ...
    BOOST_LOG_TRIVIAL(info) << "tsbk3b net stat: wacn " << std::dec << wacn << " syid " << syid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ") ";
  } else if (opcode == 0x3c) { // adjacent status
    unsigned long rfid = bitset_shift_mask(tsbk, 48, 0xff);
    unsigned long stid = bitset_shift_mask(tsbk, 40, 0xff);
    unsigned long ch1 = bitset_shift_mask(tsbk, 24, 0xffff);
    unsigned long f1 = channel_id_to_frequency(ch1, sys_num);
    BOOST_LOG_TRIVIAL(info) << "tsbk3c\tAdjacent Status\t rfid " << std::dec << rfid << " stid " << stid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ") ";
    ...
    if (f1) {
      ...
      if (it != channels[stid].end()) {
        ...
        // 1000000.0, table)
        BOOST_LOG_TRIVIAL(info) << "\ttsbk3c Chan " << temp_chan.frequency << "  " << temp_chan.step;
      }
    }
    ...
    Channel temp_chan = {
        ...
        bw * .125};
    add_channel(iden, temp_chan, sys_num);
    BOOST_LOG_TRIVIAL(info) << "tsbk3d iden id " << std::dec << iden << " toff " << toff * 0.25 << " spac " << spac * 0.125 << " freq " << freq * 0.000005;
  } else {
    BOOST_LOG_TRIVIAL(info) << "tsbk other " << std::hex << opcode;
    return messages;
  }
  messages.push_back(message);
  return messages;
...
void P25Parser::print_bitset(boost::dynamic_bitset<> &tsbk) {
  /*boost::dynamic_bitset<> bitmask(tsbk.size(), 0x3f);
     ...
     BOOST_LOG_TRIVIAL(info) << tsbk << " = " << std::hex << result;*/
}
...
std::vector<TrunkMessage> P25Parser::parse_message(gr::message::sptr msg, System *system) {
  ...
  if (type == -2) { // # request from gui
    ...
    BOOST_LOG_TRIVIAL(info) << "process_qmsg: command: " << cmd;
    ...
    BOOST_LOG_TRIVIAL(info) << "unknown message type " << type;
    messages.push_back(message);
    return messages;
  }
  ...
  {
    BOOST_LOG_TRIVIAL(info) << std::hex << "NON TBSK: nac " << nac << std::dec << " type " << type << " size " << msg->to_string().length() << " mesg len: " << msg->length();
  ...
  if (type == 7) { // # trunk: TSBK
    ...
    /*BOOST_LOG_TRIVIAL(info) << "RAW  Data    " <<b;
    BOOST_LOG_TRIVIAL(info) << "RAW  Data Length " <<s.length();*/
    BOOST_LOG_TRIVIAL(info) << "MBT:  opcode: $" << std::hex << opcode;
    /* BOOST_LOG_TRIVIAL(info) << "MBT  type :$" << std::hex << type << " len $" << std::hex << s1.length() << "/" << s2.length();
    BOOST_LOG_TRIVIAL(info) <<  "MBT Header: " <<  header;
    BOOST_LOG_TRIVIAL(info) <<  "MBT  Data   " <<  mbt_data; */
