...
SmartnetParser::print_osw(std::string s)
{
	...
	char  tempArea [512];
        ...
        // mis-interpreted if taken out of context.
				...
				//cout << "[ 0cmd: " << getfreq(stack[0].cmd) << "\t0id: " << getfreq(stack[0].id) << "\t1cmd: " << getfreq(stack[1].cmd) << "\t1id: " << getfreq(stack[1].id) << "] \t0cmd: " << stack[0].cmd << "\t0id: "<< stack[0].id << "\t1cmd: " << stack[1].cmd << "\t1id: "<< stack[1].id << "\t ( 0cmd: " << hex << stack[0].cmd << "\t 1cmd: " << hex << stack[1].cmd << " )" << endl;

				//cout << "[ Freq: " << getfreq(stack[0].cmd) << " TG: " << stack[0].id << " ] Last CMD: " << hex << stack[1].cmd << endl;
        ...
        case 0x320:
                        if(stack[0].cmd == OSW_EXTENDED_FCN)
                        {
                                ...
                                */
                                numConsumed = 3;
																cout << "something about a peer" << (stack[0].id & 0xfc00) << endl;
                                //idlesWithNoId = 0; // got identifying info...
...
/*
        ...
        if( stack[2].cmd == 0x0320 && stack[1].cmd == OSW_EXTENDED_FCN )
        {
                numConsumed = 3;
                //idlesWithNoId = 0; // got identifying info...
                if( (stack[1].id & 0xfc00) == 0x6000)
                {
										cout << "uhf/vhf equivalent of 308/320/30b" << endl;
										cout << "Freq: " << getfreq(stack[0].cmd) << " Talkgroup: " << stack[0].address << " full add: " << stack[0].full_address << "Sys: " << stack[2].id << " Source: " << stack[1].id << endl;
                }
                return;
        }
...
/*
	if( (stack[0].cmd < 0x2d0) && (stack[2].cmd == 0x0320) && (stack[1].cmd == OSW_EXTENDED_FCN) )
	{
					numConsumed = 3;
					//idlesWithNoId = 0; // got identifying info...
					//if( (stack[0].id & 0xfc00) == 0x6000)
