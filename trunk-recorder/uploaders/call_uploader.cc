...
void convert_upload_call(call_data_t *call_info, server_data_t *server_info) {
  ...
  int nchars = snprintf(shell_command, 400, "nice -n -10 ffmpeg -y -i %s  -c:a libfdk_aac -b:a 32k -filter:a \"volume=5\" -cutoff 18000 -hide_banner -loglevel error %s ", call_info->filename, call_info->converted);
