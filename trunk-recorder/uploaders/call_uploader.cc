...
void *upload_call_thread(void *thread_arg) {
  ...
  int nchars = snprintf(shell_command, 400, "sox %s -t wav - --norm=-3 | fdkaac --silent --ignorelength -m 4 -o %s -", call_info->filename, call_info->converted);
...
void send_transmission(Call *call, Transmission transmission, System *sys, Config config) {
  ...
  // call_data_t));
    ...
    call_info->freq = transmission.freq;
    ...
    call_info->start_time = transmission.start_time;
    call_info->stop_time = transmission.stop_time;
    call_info->length = transmission.length;
    ...
    Call_Source call_source = {transmission.source, transmission.start_time, 0, 0, "", ""};
    call_info->source_list.push_back(call_source);
    
    Call_Freq call_freq = {transmission.freq, transmission.start_time, 0};

      call_info->freq_list[0] = call_freq;
    
    BOOST_LOG_TRIVIAL(info) << "Trying to upload: " << call_info->converted << " Source: " << transmission.source;
...
void send_transmissions(Call *call, System *sys, Config config) {
  ...
  for (std::size_t i = 0; i < call->transmission_list.size(); i++) {
    ...
    call_info->length = call->transmission_list[i].length;
    call_info->api_key = sys->get_api_key();
    call_info->bcfy_api_key = sys->get_bcfy_api_key();
    call_info->bcfy_system_id = sys->get_bcfy_system_id();
    call_info->short_name = sys->get_short_name();
