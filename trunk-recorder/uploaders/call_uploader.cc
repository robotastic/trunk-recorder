...
void convert_upload_call(call_data_t *call_info, server_data_t *server_info) {
  ...
  int nchars = snprintf(shell_command, 400, "ffmpeg -y -i %s -c:a libfdk_aac -b:a 32k -cutoff 18000 -af 'dynaudnorm=f=250:m=50,volume=1.75' -hide_banner -loglevel panic %s ", call_info->filename, call_info->converted);
