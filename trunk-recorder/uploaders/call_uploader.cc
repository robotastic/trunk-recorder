...
void convert_upload_call(call_data_t *call_info, server_data_t *server_info) {
  ...
  if (call_info->scheme == "http") {
    BOOST_LOG_TRIVIAL(info) <<"[" << call_info->short_name <<  "]\tTG: " << call_info->talkgroup << "\tFreq: " << call_info->freq << "\tTDMA: " << call_info->phase2_tdma << "\tHTTP Upload result: " << http_upload(server_info, request_);
  }
  ...
  if (call_info->scheme == "https") {
    ...
    if (!error) {
      BOOST_LOG_TRIVIAL(info) <<"[" << call_info->short_name <<  "]\tTG: " << call_info->talkgroup << "\tFreq: " << call_info->freq << "\tTDMA: " << call_info->phase2_tdma << "\tHTTPS Upload Success - file size: " << req_size;
