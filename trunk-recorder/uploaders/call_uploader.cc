...
void convert_upload_call(call_data_t *call_info, server_data_t *server_info) {
  ...
  int nchars = snprintf(shell_command, 400, "ffmpeg -y -i %s  -c:a libfdk_aac -b:a 12k -filter:a loudnorm -hide_banner -loglevel panic %s ", call_info->filename, call_info->converted);
