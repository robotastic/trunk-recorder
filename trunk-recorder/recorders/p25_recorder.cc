...
/*  int initial_decim      = floor(samp_rate / 20000);
  ...
  int decim = floor(samp_rate / 288000);
  ...
  if (arb_rate <= 1) {
    ...
    BOOST_LOG_TRIVIAL(info) << " Resampled Rate: " << resampled_rate  << " Decimation: " << decim << " System Rate: " << system_channel_rate << " ARB Rate: " << arb_rate;
