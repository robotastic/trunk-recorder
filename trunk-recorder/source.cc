...
analog_recorder_sptr Source::create_conventional_recorder(gr::top_block_sptr tb, bool delayopen) {

    analog_recorder_sptr log = make_analog_recorder(this, delayopen);
...
void Source::create_analog_recorders(gr::top_block_sptr tb, int r) {
  ...
  for (int i = 0; i < max_analog_recorders; i++) {
    analog_recorder_sptr log = make_analog_recorder(this, true);
    analog_recorders.push_back(log);
    tb->connect(source_block, 0, log, 0);
  }
}
